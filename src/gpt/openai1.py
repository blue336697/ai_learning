from openai import OpenAI  # for openai 1.*
import datetime

from src.gpt.llm_key import openai_api_key
...
def test_stream_completions(question):
    ...
    for chunk in response:
        ...
        if first and chunk_message is not None and '，' in chunk_message:
            ...
            collected_messages.append(
                f"<本次调用chatgpt_stream首句延时：%s毫秒>" % ((chunk_time - begin_time).total_seconds() * 1000))
    ...
    print("input: %s" % (question))
    print("output: %s" % (content))
    print("流式调用：chatgpt_stream总延时：%s毫秒 \n" % ((end_time - begin_time).total_seconds() * 1000))
